...
import com.ms.taojin.manage.bo.BuyBO;
...
public class BuyService extends BaseService {
	 ...
	 * @return
	 */
	public ListRespVO list(ListReqVO<BuyEntity> reqVO) throws CenterException {
		...
		return buyBo.queryPageAutomatic(reqVO);
	}
	...
	public Object queryById(@Param("id") Long id) throws CenterException {
		SessionUser user = ThreadContext.getSessionloginUser();
		BuyEntity query = new BuyEntity();
		query.setId(id);
		query.setCreateUserId(user.getUserId());
		return buyBo.queryByEntity(query);
	}
	 ...
	 * @return
	 */
	public Object create(BuyEntity buy) throws CenterException {
		buy.setCreatetime(new Date());
		buyBo.createForValidate(buy);
		return buy;
	}
	 ...
	 * @return
	 */
	public BaseRespVO update(BuyEntity buy) throws CenterException {
		int updateCount = buyBo.updateAuthForValidate(buy);
		if(updateCount > 0){
	...
	public BaseRespVO delete(@Param("id") Long[] id) throws CenterException {
		SessionUser user = ThreadContext.getSessionloginUser();
		int deleteCount = buyBo.batchDeleteAuthById(id,user.getUserId());
		if (deleteCount > 0) {
			return new BaseRespVO();
		} else {
