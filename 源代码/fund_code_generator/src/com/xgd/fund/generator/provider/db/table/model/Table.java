...
import com.xgd.fund.generator.provider.db.table.TableFactory;
...
public class Table implements java.io.Serializable, Cloneable {
	...
	private String primaryKeyParName = null;
	...
	List<Column> varcharColumns = null;
	List primaryKeyColumns = new ArrayList<Column>();
	...
	public Table(Table t) {
		...
		this.primaryKeyName = t.getPrimaryKeyName();
	...
	public String getCreateDateColumnName() {
		if ("".equals(createDateColumnName)) {
			for (Column column : columns) {
				if ("created_date".equalsIgnoreCase(column.getSqlName()) || "f_created_date".equalsIgnoreCase(column.getSqlName())
				        || "create_date".equalsIgnoreCase(column.getSqlName()) || "f_create_date".equalsIgnoreCase(column.getSqlName())
				        || "create_time".equalsIgnoreCase(column.getSqlName()) || "f_create_time".equalsIgnoreCase(column.getSqlName())
				        || "createTime".equalsIgnoreCase(column.getSqlName()) || "createDate".equalsIgnoreCase(column.getSqlName())) {
					this.createDateColumnName = column.getSqlName();
					break;
				}
			}
	...
	public String getPrimaryKeyParName() {
		return primaryKeyParName;
	}

	public String getPrimaryKeyParNameUp() {
		return primaryKeyParName.substring(0, 1).toUpperCase() + primaryKeyParName.substring(1);
	}

	public String getPrimaryKeyNameUp() {
		return primaryKeyName.substring(0, 1).toUpperCase() + primaryKeyName.substring(1);
	}
	...
	public void setPrimaryKeyName(String primaryKeyName) {
		...
		this.primaryKeyParName = StringHelper.uncapitalize(primaryKeyName.replaceAll("(?i)F_", ""));
		this.primaryKeyParName = StringHelper
		        .uncapitalize(StringHelper.makeAllWordFirstLetterUpperCase(StringHelper.toUnderscoreName(primaryKeyName.replaceAll("(?i)F_", ""))));
	}
	...
	public List<Column> getVarcharColumns() {
		if (varcharColumns == null) {
			varcharColumns = new ArrayList<Column>(columns);
			Iterator<Column> iter = varcharColumns.iterator();
			while (iter.hasNext()) {
				if (!"VARCHAR".equalsIgnoreCase(iter.next().getJdbcType())) {
					iter.remove();
				}
	...
	public String getSuffixTableName() {
		String tableName = getSqlName();
		if (tableName.startsWith("T_FM_")) {
	...
	public String getRemarks() {
		if (!StringHelper.isEmpty(remarks)) {
			...
			if (trim.endsWith("表")) {
				remarks = trim.substring(0, trim.length() - 1);
			}
		}
		return remarks;
	}
	 ...
	 * @return
	 */
	public String getClassNameEntity() {
		String _className = getClassName();
	...
	public String getPkType() {
		return getPkColumn().getPossibleShortJavaType();
	}
	...
	public String getAuthor() {
		return GeneratorProperties.getRequiredProperty("code.author");
	}
