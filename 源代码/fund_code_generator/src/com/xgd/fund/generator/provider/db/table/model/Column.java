package com.xgd.fund.generator.provider.db.table.model;

import java.util.ArrayList;
import java.util.List;

import com.xgd.fund.generator.GeneratorProperties;
import com.xgd.fund.generator.provider.db.table.model.ForeignKey.ReferenceKey;
import com.xgd.fund.generator.provider.db.table.model.util.ColumnHelper;
import com.xgd.fund.generator.util.GLogger;
import com.xgd.fund.generator.util.StringHelper;
import com.xgd.fund.generator.util.TestDataGenerator;
import com.xgd.fund.generator.util.typemapping.ActionScriptDataTypesUtils;
import com.xgd.fund.generator.util.typemapping.DatabaseDataTypesUtils;
import com.xgd.fund.generator.util.typemapping.JavaPrimitiveTypeMapping;
import com.xgd.fund.generator.util.typemapping.JdbcType;
...
public class Column implements java.io.Serializable, Cloneable {
	 ...
	 * @param remarks
	 */
	public Column(Table table, int sqlType, String sqlTypeName, String sqlName, int size, int decimalDigits, boolean isPk, boolean isNullable,
	        boolean isIndexed, boolean isUnique, String defaultValue, String remarks) {
		if (sqlName == null)
	...
	public Column(Column c) {
		this(c.getTable(), c.getSqlType(), c.getSqlTypeName(), c.getSqlName(), c.getSize(), c.getDecimalDigits(), c.isPk(), c.isNullable(), c.isIndexed(),
		        c.isUnique(), c.getDefaultValue(), c.getRemarks());
	}
	...
	public String getNickName() {
		String nickName = getRemarks();

		int splitIndex = nickName.length();
		int thenIndex = 0;

		List<String> splitVar = new ArrayList<String>();
		splitVar.add(":");
		splitVar.add(" ");
		splitVar.add(",");
		splitVar.add("(");
		...
		for (String str : splitVar) {
			thenIndex = nickName.indexOf(str);
			if (0 < thenIndex && thenIndex < splitIndex) {
				splitIndex = thenIndex;
			}
		}

		if (splitIndex < nickName.length()) {
			nickName = nickName.substring(0, splitIndex);
		}
	...
	// _table.getImportedKeys().addForeignKey(ref.tableName, ref.columnSqlName, getSqlName(),
	// ref.columnSqlName.hashCode());
	// }
	// }
	//
	...
	public void setHasOne(String foreignKey) {
		...
		if (hasOne != null && _table != null) {
			...
			// _table.getImportedKeys().addForeignKey(refTable.getSqlName(), hasOne.columnSqlName,
			// getSqlName(), hasOne.columnSqlName.toLowerCase().hashCode());
	...
	public void setHasMany(String foreignKey) {
		...
		if (hasMany != null && _table != null) {
			...
			// _table.getExportedKeys().addForeignKey(refTable.getSqlName(), hasMany.columnSqlName,
			// getSqlName(), hasMany.columnSqlName.toLowerCase().hashCode());
	...
	private void initOtherProperties() {
		...
		columnName = StringHelper.makeAllWordFirstLetterUpperCase(StringHelper.toUnderscoreName(getSqlName().replaceAll("(?i)F_", "")));
