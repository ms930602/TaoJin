package com.xgd.fund.generator.util;
...
import com.xgd.fund.generator.provider.db.table.model.Column.EnumMetaDada;
...
public class StringHelper {
	...
	public static String insertBefore(String content, String compareToken, String insertString) {
		...
		if (index >= 0) {
			...
			throw new IllegalArgumentException("not found insert location by compareToken:" + compareToken
			        + " content:" + content);
		}
	}
	...
	public static List<EnumMetaDada> string2EnumMetaData(String data) {
		...
		while (m.find()) {
			...
			throw new IllegalArgumentException("error enumString format:" + data
			        + " expected format:F(1,Female);M(0,Male) or F(Female);M(Male)");
		}
		return list;
	}
